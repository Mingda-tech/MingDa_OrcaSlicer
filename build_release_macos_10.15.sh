...
DEPS="$DEPS_BUILD_DIR/mingda-slicer_dep_$ARCH"
...
function pack_deps() {
    ...
    (
        ...
        mkdir -p "$DEPS"
        cd "$DEPS_BUILD_DIR"
        tar -zcvf "mingda-slicer_dep_mac_${ARCH}_$(date +"%Y%m%d").tar.gz" "mingda-slicer_dep_$ARCH"
    )
}
...
function build_slicer() {
    ...
    (
        ...
        if [ "1." != "$BUILD_ONLY". ]; then
            cmake .. \
                ...
                -DCMAKE_INSTALL_PREFIX="$PWD/mingda-slicer" \
                -DCMAKE_BUILD_TYPE="$BUILD_CONFIG" \
    ...
    (
        cd "$PROJECT_BUILD_DIR"
        mkdir -p mingda-slicer
        cd mingda-slicer
        # remove previously built app
        rm -rf ./mingda-slicer.app
        # fully copy newly built app
        cp -pR "../src$BUILD_DIR_CONFIG_SUBDIR/mingda-slicer.app" ./mingda-slicer.app
        # fix resources
        resources_path=$(readlink ./mingda-slicer.app/Contents/Resources)
        rm ./mingda-slicer.app/Contents/Resources
        cp -R "$resources_path" ./mingda-slicer.app/Contents/Resources
        # delete .DS_Store file
        find ./mingda-slicer.app/ -name '.DS_Store' -delete
    )
    ...
    # zip -FSr mingda-slicer${ver}_Mac_${ARCH}.zip mingda-slicer.app
}
